...
pub fn process_image(image_data: &[u8], format: &str) -> Vec<u8> {
    ...
    if image_format == ImageFormat::Jpeg {
        process_jpeg(resized_image)
    } else {
        process_png(resized_image)
    }
}

fn process_jpeg(resized_image: DynamicImage) -> Vec<u8> {
    ...
    let mut collage = ImageBuffer::from_pixel(collage_width, collage_height, Rgb([255, 255, 255]));
...
fn process_png(resized_image: DynamicImage) -> Vec<u8> {
    let (id_photo_width, id_photo_height) = resized_image.dimensions();
    let margin = 10; // Adjust spacing as needed
    ...
    let mut output_data = Vec::new();
    let mut output_cursor = std::io::Cursor::new(&mut output_data);
    collage.write_to(&mut output_cursor, ImageFormat::Png).unwrap();
